...
class LoadDatabase {
	...
	CommandLineRunner initDatabase(RecipeRepository recRepo, JournalEntryRepository journRepo, AnnotationRepository annotRepo) {
		...
		instr.addAnnotation(new InstructionAnnotation(1, "Im an annotation"));
		rec2.addStep(instr);
		rec2.setUserID(1);
		recRepo.save(rec2);
		
		JournalEntry journ = new JournalEntry(rec2, "vegan", 25, 35, 3, 5, new IngredientAnnotation(3, "ingredient annotation"));
